...
def create_heart_button():
    """Create a stylized heart button with CSS animation"""
    heart_button_html = """
    <style>
    .heart-button {
        position: relative;
    ...
    .heart-button:hover .button-text {
    ...
    .heart-button:hover {
        animation: scaleUp 0.3s ease-in-out;
        transform: translateY(-2px);
        box-shadow: 0 5px 15px rgba(253, 24, 83, 0.4);
    }
    ...
    .heart-button:hover .star-1 {
        top: -10%;
        left: -5%;
        filter: drop-shadow(0 0 8px #fd1853);
        z-index: 2;
    }

    .heart-button:hover .star-2 {
        top: 34%;
        left: 41.5%;
        filter: drop-shadow(0 0 8px #fd1853);
        z-index: 2;
    ...
    <button class="heart-button" onclick="triggerFullScreenHeartAnimation()" type="button">
        <span class="button-text">💕 LOVE</span>
        ...
        <div class="star-3">
            <svg height="6" width="6" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                <path d="M427.313,88.686c-47.803-47.803-125.213-47.803-173.016,0l-17.087,17.087l-17.087-17.087
                    c-47.803-47.803-125.213-47.803-173.016,0c-47.803,47.803-47.803,125.213,0,173.016l190.103,190.103
                    c4.88,4.88,11.316,7.322,17.752,7.322c6.435,0,13.871-2.442,18.751-7.322l190.103-190.103
                    C475.116,213.899,475.116,136.489,427.313,88.686z" fill="#fd1853"></path>
        ...
        <div class="star-4">
            <svg height="8" width="8" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                <path d="M427.313,88.686c-47.803-47.803-125.213-47.803-173.016,0l-17.087,17.087l-17.087-17.087
                    c-47.803-47.803-125.213-47.803-173.016,0c-47.803,47.803-47.803,125.213,0,173.016l190.103,190.103
                    c4.88,4.88,11.316,7.322,17.752,7.322c6.435,0,13.871-2.442,18.751-7.322l190.103-190.103
                    C475.116,213.899,475.116,136.489,427.313,88.686z" fill="#fd1853"></path>
    ...
    <script>
    function triggerFullScreenHeartAnimation() {
        // Try to access the main window (parent of the iframe)
        let targetWindow = window;
        let targetDocument = document;
        
        // Check if we're in an iframe and try to access parent
        if (window.parent && window.parent !== window) {
            try {
                targetWindow = window.parent;
                targetDocument = window.parent.document;
            } catch (e) {
                // If we can't access parent, fall back to current window
                console.log("Cannot access parent window, using current window");
            }
        }
        
        // Create fullscreen container
        const heartContainer = targetDocument.createElement('div');
        heartContainer.style.cssText = `
            ...
            overflow: hidden;
        `;
        heartContainer.id = 'fullscreen-hearts-' + Date.now();
        
        // Append to body
        targetDocument.body.appendChild(heartContainer);
        
        // Create CSS animation keyframes in the target document
        ...
        for (let i = 0; i < 40; i++) {
            setTimeout(() => {
                const heart = targetDocument.createElement('div');
            ...
            }, i * 80); // Stagger creation by 80ms
        }
        
        // Remove container and style after animation completes
        setTimeout(() => {
    ...
    components.html(heart_button_html, height=60)

def show_heart_animation():
    """Display floating hearts animation for 3 seconds using Streamlit session state trigger"""
    ...
    function showHearts() {
        ...
        if (container) {
            // Create 35 hearts with staggered timing
            for (let i = 0; i < 35; i++) {
                setTimeout(() => {
    ...
    import streamlit.components.v1 as components
    components.html(heart_animation_html, height=0)
...
def updated_main():
    ...
    st.title("🏥 Russian Learning App for My Soon-to-Be Doctor")
    
    # Subtitle with heart button next to it
    subtitle_col, button_col = st.columns([3, 1])
    with subtitle_col:
        st.markdown("*Made with love and care*")
    with button_col:
        create_heart_button()
